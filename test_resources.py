...
def test_resources():
    ...
    print("-" * 30)
    config = Config()
    config.load(str(res / 'synthea.properties'))
    ...
    print("-" * 30)
    immun_file = res / 'immunization_schedule.json'
    ...
    print("-" * 30)
    growth_file = res / 'cdc_growth_charts.json'
    ...
    for ext in ['*.json', '*.csv', '*.yml', '*.yaml']:
        total_files += len(list(res.rglob(ext)))

    print(f"✓ Total resource files: {total_files}")
    print(f"✓ Disease modules: {len(modules)}")

    subdirs = [d for d in res.iterdir() if d.is_dir()]
